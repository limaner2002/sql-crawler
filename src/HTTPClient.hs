...
login mgr = do
  (cookies, _) <- makeReq mgr mempty baseHeaders dbReq -- get CSRF token
  let req = prepareReq cookies baseHeaders (setQueryString (getCSRFToken cookies <> authParams) <$> authReq)

  (cookies', _, _) <- join $ makeReq' mgr <$> addRequestHeader "Content-Type" "application/x-www-form-urlencoded" <$> req -- Login
  
  makeReq mgr cookies' baseHeaders dbReq -- DB Login and get SSO token
...
getReferencingTables tName = do
  (_, body) <- doIt $ "SELECT * FROM KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = '" <> tName <> "';"
  r <- join <$> (sequence $ getResult <$> body `index` 1)

  return $ fmap keyColumnUsage <$> drop 1 <$> filter (/= mempty) <$> r

printReferencingTables tName = do
  referencingTables <- getReferencingTables tName
  case referencingTables of
    Just [] -> putStrLn $ "No tables have foreign-keys pointing to " <> tshow tName
    Nothing -> putStrLn $ "There was an error that should not have happened."
    _ -> (mapM_ . mapM_ . mapM_) (print . tableName) referencingTables

printReferencingTables' :: Maybe [Either SomeException KeyColumnUsage] -> IO ()
printReferencingTables' (Just []) = putStrLn $ "No foreign-key relations."
printReferencingTables' Nothing = putStrLn $ "There was an error that should not have happened."
printReferencingTables' referencingTables = (mapM_ . mapM_ . mapM_) (print . tableName) referencingTables
